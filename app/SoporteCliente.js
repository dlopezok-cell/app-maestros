...
export default function SoporteCliente({ usuario, onBack }) {
  ...
  function cargar() {
    if (!usuario) return;
    supabase.from('mensajes_soporte').select('*').eq('cliente_id', usuario.id).order('creado_en', { ascending: true })
      .then(function (r) {
        setMsgs(r.data || []); setCargado(true);
        supabase.from('mensajes_soporte').update({ leido: true }).eq('cliente_id', usuario.id).eq('autor', 'admin').eq('leido', false).then(function () {});
  ...
  function enviar() {
    var t = txt.trim(); if (!t || !usuario) return;
    setEnviando(true);
    supabase.from('mensajes_soporte').insert({ cliente_id: usuario.id, autor: 'cliente', texto: t }).then(function (r) {
      setEnviando(false);
  ...
  return (
    <div style={vp
      ...
      <div onTouchStart={function () { if (inpRef.current) inpRef.current.blur(); }} style={{ flex: 1, overflowY: 'auto', background: '#f5f6fa', padding: 16 }}>
        ...
        {cargado && msgs.length === 0 && <div style={{ background: '#fff', borderRadius: 14, padding: '10px 13px', fontSize: 13.5, color: '#1c1f2b', maxWidth: '82%', boxShadow: '0 1px 1px rgba(0,0,0,.05)' }}>¡Hola! ¿En qué te podemos ayudar? Escríbenos cualquier duda sobre tus pedidos, pagos o cómo usar la app.</div>}
        {msgs.map(function (m) {
          var out = m.autor === 'cliente';
          return (
      ...
      <div style={{ display: 'flex', gap: 8, padding: '10px 12px', paddingBottom: 'calc(24px + env(safe-area-inset-bottom))', borderTop: '1px solid #eee', background: '#fff', alignItems: 'center' }}>
        ...
        <button onMouseDown={function (e) { e.preventDefault(); }} onClick={enviar} disabled={enviando} style={{ width: 42, height: 42, borderRadius: '50%', background: '#2563eb', color: '#fff', border: 'none', fontSize: 16, cursor: 'pointer' }}>{'➤'}</button>
      </div>
    </div>
  );
}
