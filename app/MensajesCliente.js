...
export default function MensajesCliente({ usuario, maestros }) {
  ...
  if (cargando) return <div style={{ padding: '24px 16px', color: '#9aa1b5', fontSize: 13 }}>Cargando conversaciones…</div>;

  if (!hilos.length) return (
    <div style={{ textAlign: 'center', padding: '50px 30px', color: '#9aa1b5' }}>
      <div style={{ fontSize: 40, marginBottom: 10 }}>{'\u{1F4AC}'}</div>
      <div style={{ fontSize: 14, fontWeight: 800, color: '#5b6275' }}>Aún no tienes mensajes</div>
      <div style={{ fontSize: 12.5, marginTop: 6, lineHeight: 1.5 }}>Cuando converses con un maestro desde tus cotizaciones, tus chats aparecerán aquí.</div>
    </div>
  );
