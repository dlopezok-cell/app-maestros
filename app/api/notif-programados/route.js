...
async function run() {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const mr = await sb.from('maestros').select('nombre, telefono').eq('id', row.maestro_id).maybeSingle();
    const p = pr.data, m = mr.data;
    if (!p || !m || !m.telefono) { await sb.from('wa_notif').update({ estado: 'error' }).eq('id', row.id); continue; }
