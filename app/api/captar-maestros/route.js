...
export async function POST(req) {
  try {
    ...
    const pr = await sb.from('presupuestos').select('id,oficio,comuna,creado_en').eq('id', pid).maybeSingle();
    const p = pr.data;
    ...
    for (let i = 0; i < places.length && rows.length < max; i++) {
      const pl = places[i];
      try {
        ...
        const mensaje = tmpl.replace(/\{nombre\}/g, (nombre.split(' ')[0] || ''))
          .replace(/\{oficio\}/g, p.oficio).replace(/\{comuna\}/g, p.comuna).replace(/\{link\}/g, LINK);
        rows.push({ presupuesto_id: pid, oficio: p.oficio, comuna: p.comuna, nombre: nombre, telefono: tel, whatsapp: w, direccion: det.formatted_address || '', es_movil: esMovil(w), mensaje: mensaje, estado: 'pendiente' });
      } catch (e) { /* salta */ }
    }
