...
export default function PresupuestoCliente({ usuario, maestros }) {
  ...
  function cargarSolicitudes() {
    ...
    supabase.from('presupuestos').select('*, cotizaciones(*)').eq('cliente_id', usuario.id).order('creado_en', { ascending: false })
      .then(function (r) { setSolicitudes(r.data || []); });
  }
  ...
  return (
    <div className="body" style={{ paddingTop: 18 }}>
      <div style={card}>
        ...
        <div style={{ fontSize: 12, color: '#7c8499', margin: '4px 0 12px' }}>Graba un video corto mostrando el problema. Un maestro lo revisa y te manda una solución o presupuesto, sin tener que coordinar una videollamada.</div>
        ...
        <button className="gbtn full" style={{ opacity: subiendo ? .6 : 1 }} disabled={subiendo} onClick={enviar}>{subiendo ? 'Enviando...' : 'Enviar y pedir presupuesto'}</button>
      </div>
          ...
          return (
            <div key={s.id} style={{ borderTop: '1px solid #f1f1f1', padding: '12px 0' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <span style={{ fontSize: 11, padding: '3px 9px', borderRadius: 8, background: cots.length ? '#f2fbf6' : '#fff9f0', color: cots.length ? '#0d9456' : '#b07a1e', fontWeight: 800 }}>{cots.length ? cots.length + ' COTIZACIÓN' + (cots.length > 1 ? 'ES' : '') : 'ESPERANDO'}</span>
              </div>
              ...
              {cots.map(function (c) {
                return (
                  <div key={c.id} style={{ background: '#f7f9fc', borderRadius: 12, padding: 10, marginTop: 8 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <b style={{ fontSize: 13 }}>{nombreMaestro(c.maestro_id)}</b>
                      {c.monto ? <b style={{ fontSize: 14, color: '#0d9456' }}>{plata(c.monto)}</b> : null}
                    </div>
                    {c.mensaje && <div style={{ fontSize: 12, color: '#5b6275', marginTop: 2 }}>{c.mensaje}</div>}
                  </div>
                );
              })}
            </div>
