...
export default function CampanaMaestros() {
  ...
  // Envío por API oficial (WhatsApp Cloud API de Meta)
  const [apiTemplate, setApiTemplate] = useState('');
  const [apiLang, setApiLang] = useState('es');
  const [apiVars, setApiVars] = useState('{remitente}');
  ...
  useEffect(function () {
    ...
    try {
      var t = localStorage.getItem('mel_api_tmpl'); if (t) setApiTemplate(t);
      var l = localStorage.getItem('mel_api_lang'); if (l) setApiLang(l);
      var v = localStorage.getItem('mel_api_vars'); if (v) setApiVars(v);
    } catch (e) {}
  }, []);
  ...
  // ---- Envío por la API oficial de WhatsApp (plantillas aprobadas en Meta) ----
  function guardarApi(campo, valor) {
    try { localStorage.setItem(campo, valor); } catch (e) {}
  }
  // Marca estado usando actualización funcional (seguro dentro de un bucle async)
  function marcar(w, s) {
    setEstado(function (prev) {
      var ns = Object.assign({}, prev); ns[w] = s;
      try { localStorage.setItem(STORE, JSON.stringify(ns)); } catch (e) {}
      return ns;
    });
  }
  function paramsDe(c) {
    var rm = (remit || 'Andrea').trim();
    return (apiVars || '').split(',').map(function (s) { return s.trim(); }).filter(Boolean).map(function (v) {
      return v.replace(/{remitente}/g, rm).replace(/{nombre}/g, (c.n || '').split(' ')[0] || 'maestro');
    });
  }
  ...
  async function enviarAPI(c, jwt) {
    try {
      var token = jwt || (await jwtAdmin());
      if (!token) { return { ok: false, error: 'Inicia sesión como admin' }; }
      var r = await fetch('/api/wa-send', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token },
        body: JSON.stringify({ to: c.w, template: apiTemplate.trim(), lang: apiLang.trim(), params: paramsDe(c) })
      });
      var d = await r.json();
      if (d.ok) { marcar(c.w, 'sent'); return { ok: true }; }
      return { ok: false, error: d.error || 'desconocido' };
    } catch (e) { return { ok: false, error: e.message }; }
  }
  async function enviarUno(c) {
    if (!apiTemplate.trim()) { alert('Primero escribe el nombre de tu plantilla aprobada (sección "Envío por API").'); return; }
    setEnviando(c.w);
    var res = await enviarAPI(c);
    setEnviando('');
    if (!res.ok) alert('No se pudo enviar a ' + c.n + ': ' + res.error);
  }
  async function enviarPaqueteAPI(i) {
    if (!apiTemplate.trim()) { alert('Primero escribe el nombre de tu plantilla aprobada (sección "Envío por API").'); return; }
    var pk = paquetes[i];
    var pend = pk.filter(function (c) { return getSt(c) === 'pend'; });
    if (!pend.length) { alert('No hay contactos pendientes en este paquete.'); return; }
    if (!confirm('Se enviarán ' + pend.length + ' mensajes por la API de WhatsApp con la plantilla "' + apiTemplate.trim() + '".\n\n¿Continuar?')) return;
    var jwt = await jwtAdmin();
    if (!jwt) { alert('Inicia sesión como admin.'); return; }
    var fallidos = 0;
    for (var k = 0; k < pend.length; k++) {
      setEnviando(pend[k].w);
      var res = await enviarAPI(pend[k], jwt);
      if (!res.ok) { fallidos++; if (fallidos <= 2) alert('Error al enviar a ' + pend[k].n + ': ' + res.error); }
      await new Promise(function (rs) { setTimeout(rs, 1500); });
    }
    setEnviando('');
    alert('Envío del paquete ' + (i + 1) + ' terminado. Enviados: ' + (pend.length - fallidos) + ' · Con error: ' + fallidos);
  }
  function exportar() {
  ...
  return (
    <div style={{ maxWidth: 1100 }}>
      ...
      {/* Envío por API oficial (WhatsApp Cloud · Meta) */}
      <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 14, padding: 16, marginBottom: 16 }}>
        <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 4 }}>{'\u{1F7E2}'} Envío por API oficial (WhatsApp Cloud · Meta)</div>
        <div style={{ fontSize: 12, color: '#6b7280', marginBottom: 10 }}>Envía tu plantilla aprobada directo desde aquí. El token se configura en Vercel (no se guarda en el navegador). En las variables puedes usar {'{remitente}'} y {'{nombre}'}.</div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, alignItems: 'flex-end' }}>
          <div><label style={lbl}>Nombre de plantilla</label><input value={apiTemplate} onChange={function (e) { setApiTemplate(e.target.value); guardarApi('mel_api_tmpl', e.target.value); }} placeholder="ej: invitacion_maestros" style={Object.assign({ width: 220 }, input)} /></div>
          <div><label style={lbl}>Idioma</label><input value={apiLang} onChange={function (e) { setApiLang(e.target.value); guardarApi('mel_api_lang', e.target.value); }} placeholder="es / es_CL" style={Object.assign({ width: 90 }, input)} /></div>
          <div style={{ flex: 1, minWidth: 220 }}><label style={lbl}>Variables del cuerpo (coma)</label><input value={apiVars} onChange={function (e) { setApiVars(e.target.value); guardarApi('mel_api_vars', e.target.value); }} placeholder="{remitente}" style={Object.assign({ width: '100%' }, input)} /></div>
        </div>
        <div style={{ fontSize: 11, color: '#9ca3af', marginTop: 8 }}>Si tu plantilla no tiene variables, deja "Variables" en blanco. El orden debe coincidir con {'{{1}}'}, {'{{2}}'}… de tu plantilla.</div>
      </div>
          ...
          <div key={i} style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 14, marginBottom: 16, overflow: 'hidden' }}>
            <div onClick={function () { var a = Object.assign({}, abiertos); a[i] = !abierto; setAbiertos(a); }} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '14px 18px', cursor: 'pointer', background: '#fafafa', borderBottom: '1px solid #e5e7eb' }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={function (e) { e.stopPropagation(); enviarPaqueteAPI(i); }} style={{ background: '#075E54', color: '#fff', border: 'none', borderRadius: 9, padding: '9px 12px', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>Enviar por API</button>
                <button onClick={function (e) { e.stopPropagation(); siguiente(i); }} style={{ background: '#25D366', color: '#063d2b', border: 'none', borderRadius: 9, padding: '9px 12px', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>{'▶'} Abrir uno</button>
                      ...
                      return (
                        <tr key={c.w} style={{ borderBottom: '1px solid #f1f1f1' }}>
                          ...
                          <td style={{ padding: '9px 14px', whiteSpace: 'nowrap' }}>
                            <a href={wa(c)} target="_blank" rel="noreferrer" onClick={function () { marcarEnviado(c.w); }} style={{ border: '1px solid #e5e7eb', borderRadius: 8, padding: '6px 10px', fontSize: 12, color: '#075E54', fontWeight: 700, textDecoration: 'none' }}>WhatsApp {'→'}</a>
                            <button onClick={function () { enviarUno(c); }} disabled={enviando === c.w} title="Enviar por API oficial" style={{ marginLeft: 6, border: 'none', borderRadius: 8, padding: '6px 9px', fontSize: 12, fontWeight: 700, cursor: 'pointer', background: enviando === c.w ? '#9ca3af' : '#075E54', color: '#fff' }}>{enviando === c.w ? '…' : 'API'}</button>
                          </td>
                        </tr>
                      );
                    })}
