...
export default function CampanaMaestros() {
  ...
  var regiones = Array.from(new Set(CONTACTS.map(function (c) { return regionDe(c.z); }))).sort();
  var comunas = Array.from(new Set(CONTACTS.map(function (c) { return limpiaComuna(c.c); }).filter(function (x) { return x !== '—'; }))).sort();
